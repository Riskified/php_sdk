...
/**
 * Riskified Order Submit Example
 * 
 * This example demonstrates how to submit an order using the Riskified PHP SDK.
...
$order->line_items = [$lineItem1, $lineItem2, $lineItem3];
...
echo "\nORDER REQUEST:" . PHP_EOL .$order->toJson() . PHP_EOL;
...
try {
    ...
    echo PHP_EOL . "Order submission successful. Riskified response: " . PHP_EOL . 
    ...
    echo PHP_EOL . "Order submission failed. HTTP Status: " . $unsuccessfulException->statusCode . 
         PHP_EOL . "Response body: " . json_encode($unsuccessfulException->jsonResponse, JSON_PRETTY_PRINT) . PHP_EOL;
} catch (Exception $generalException) {
    echo PHP_EOL . "Order submission encountered an error: " . $generalException->getMessage() . PHP_EOL;
}
